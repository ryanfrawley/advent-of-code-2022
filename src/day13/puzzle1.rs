...
pub fn run() {
    ...
    loop {
        ...
        let (left, right) = (lines.next().unwrap(), lines.next().unwrap());
        println!("---");
        println!("Left: {left}");
        println!("Right: {right}");

        if correct_order(left, right, 1) != Ordering::Greater {
            println!("Pair {pair} is correct");
            pair_sum += pair;
        }
        pair += 1;
    }
    println!("Correct pair sum: {pair_sum}");
}
...
fn correct_order(left: &str, right: &str, depth: u32) -> Ordering {
    ...
    loop {
        ...
        if lval == None || lval == Some("") {
            ...
            println!("Left ran out at depth {depth}");
            return Ordering::Less;
        }

        // Empty right case
        if rval == None || rval == Some("") {
            println!("Right ran out at depth {depth}");
...
mod tests {
    ...
    use crate::day13::puzzle1::correct_order;
